...
OS_VERSION=`cat /etc/*release |grep ID_LIKE | awk -F\" '{print $2}' | awk -F " " '{print $1}'`
#OS_TYPE=`cat /etc/*release|head -1|awk '{print $1}'`
OS_TYPE=`cat /proc/version | awk -F "Red Hat" '{print $2}' | awk -F "." '{print $1}'`
...
if 
  [[ ${OS_TYPE} -eq 7 ]] && [[ ${OS_VERSION} == centos ]]
 then
...
sed -i 's/gpgcheck=1/gpgcheck=0/g' /etc/yum.repos.d/jenkins.repo # this should be 0
